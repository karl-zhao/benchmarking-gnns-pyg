...
def train_val_pipeline(MODEL_NAME, dataset, params, net_params, dirs):
    ...
    if not net_params['edge_feat']:
        edge_feat_dim = 1
        dataset.edge_attr = torch.ones(dataset.dataset[0].num_edges, edge_feat_dim).type(torch.float32)
    ...
    if DATASET_NAME == 'ogbn-arxiv' or DATASET_NAME == 'ogbn-mag':                    # , 'ogbn-proteins''
    ...
    try:
        with tqdm(range(params['epochs']),ncols= 0) as t:
            for epoch in t:
                ...
                epoch_train_loss = train_epoch(model, optimizer, device, dataset, dataset.split_idx['train'])
